   ...
   case '2':
    {   cout << "\t\t     THANK YOU FOR USING THIS SOFTWARE"<<endl;
        ...
        cout << "\n\n";
             ...
             cout << "\t   1. Rohit Kumar verma             GUE/01182/11          +251974461688   \n\n";
             cout << "\t   2. Saurabh                       GUE/01446/11          +251926489069    \n\n";
             cout << "\t   3. Kumar Adyut                   GUE/01479/11          +251941257770     \n\n";
...

{
    ...
    while(infile.read(reinterpret_cast<char*>(&s),sizeof(student)))
    {
        ...
        infile.close();
        if(equality==false) //koi nhi mila
...

{
    ...
    if(infile.fail())
    {
     ...
     while(!infile.eof() && checker==false) //eof end of file
     {
    ...
    s.sum= s.pro+ s.arc+ s.writing+ s.alg;
    s.average=(s.sum/4);
    int pos=(-1)*static_cast<int>(sizeof(student)); //set the pointer at pos position
